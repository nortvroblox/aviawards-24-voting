import { useDebugValue, useEffect, useMemo, useRef } from "@rbxts/react";
...
export function useSyncExternalStoreWithSelector<Snapshot, Selected>(
	...
	if (instReference.current === undefined)
		instReference.current = inst = { hasValue: false, value: undefined };
	...
	const getSelection = useMemo(() => {
		...
		const memoizedSelector: Selector<Snapshot, Selected> = (
			nextSnapshot
		) => {
			if (!hasMemo) {
				hasMemo = true;
			...
			if (is(previousSnapshot, nextSnapshot))
				return previousSelection as Selected;

			const nextSelection = selector(nextSnapshot);
			if (isEqual?.(previousSelection as Selected, nextSelection))
				return previousSelection as Selected;
		...
		const getSnapshotWithSelector = (): Selected =>
