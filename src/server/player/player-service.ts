...
export default class PlayerService implements OnStart {
	private readonly playerLeaveEvents = new Array<
		ListenerData<OnPlayerLeave>
	>();

	private readonly onEntityJoined = new Signal<
	...
	public onStart(): void {
		...
		onPlayerAdded((player) => {
			this.onPlayerJoin(player).catch((err) => {
				this.logger.Error(
					`Failed to load player ${player.UserId}: ${err}`
				);
			});
		});
		...
		Players.PlayerRemoving.Connect(
			this.withPlayerEntity((playerEntity) => {
				this.onPlayerRemoving(playerEntity).catch((err) => {
					this.logger.Error(
						`Failed to close player ${playerEntity.userId}: ${err}`
					);
				});
			})
		);
	 ...
	 *   not found.
	 */
	public async getPlayerEntityAsync(
		player: Player
	...
	private async onPlayerJoin(player: Player): Promise<void> {
		const playerProfile = await this.playerDataService.loadProfile(player);
		if (!playerProfile) {
			this.playerRemovalService.removeForBug(
				player,
				KickCode.PlayerInstantiationError
			);
			return;
		}

		const janitor = this.setupPlayerJanitor(player);
		...
		debug.profilebegin("Lifecycle_Player_Join");
		{
			for (const { id, event } of this.playerJoinEvents) {
				janitor
					...
					.catch((err) => {
						this.logger.Error(
							`Error in player lifecycle ${id}: ${err}`
						);
					});
			}
		}
	...
	/**
	 * Private setupPlayerJanitor(player: Player, playerDocument:
	 * Document<PlayerData>): Janitor {.
	 *
	 * @param player
	 */
	private setupPlayerJanitor(player: Player): Janitor {
		const janitor = new Janitor();
		janitor.Add(async () => {
			this.logger.Info(
				`Player ${player.UserId} leaving game, cleaning up Janitor`
			);

			try {
				// await playerDocument.close();
				this.playerDataService.unloadProfile(player);
			} catch (err) {
				this.logger.Error(
					`Failed to close player document for ${player.UserId}: ${err}`
				);
			}
	...
	private bindHoldServerOpen(): void {
		...
		game.BindToClose(() => {
			...
			this.logger.Debug(
				`Game closing, holding open until all player entities are removed.`
			);
